...
const MobileViewSection: React.FC<Props> = ({ data, onChange, isPreview = false }) => {
  ...
  const handleDownloadPDF = async () => {
    setPdfLoading(true);
    try {
      const base64 = await handlePdfAction(finalHtml, 'mobile');
  ...
  return (
    <div className={`${isPreview ? '' : 'bg-white p-4'} w-full flex item-cemter flex-col`}>
      ...
      <div className='bg-white min-h-[600px]  flex justify-center'>
        <div className='w-full max-w-[375px] border border-gray-200 shadow-lg  overflow-hidden  bg-white'>
